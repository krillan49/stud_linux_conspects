...
# 5. cut - команда которая может разбивать текст по символу, нумерует каждый результат и выводит заданные
...
# $ grep ark ./*                        - находит ark во всех фаилах в текущей директории
# $ grep -n -i main main.cpp            - находит слово "main" в файле main.cpp. Выводит номер строки и не учитывает регистр


# С командой grep можно использовать Регулярные выражения, тк папаметр поиска по умолчанию рассматривается как регулярное выражение, но по умолчанию не все операторы регулярок работают некорректно.
...
# $ grep "[A-Z]{1,3}[a-z]*bc" ./*          - находит все части текста соотв регулярке во всех фаилах текущей директории
...
# $ grep "[A-Z]{1,3}[a-z]*bc" ./* | wc      - с регуляркой



echo "                               Потоки вывода. Перенаправление ввода/вывода"

# > - команда перенаправляет вывода консоли(не фаила) в фаил, перезаписывает его полностью, те предварительно стирает все содержимое, создает новый фаил если он не существует
...
# >> - команда перенаправляет вывода в другой фаил, добавляет в конец, те дозаписывает, сохраняя все содержимое, создает новый фаил если он не существует.
...
# $ grep krillan /etc/*                        - при поиске слова krillan в системных директориях, часть строк будет закрыто "Permission denied" и другими сообщениями об ошибках, это поток "плохого вывода"
