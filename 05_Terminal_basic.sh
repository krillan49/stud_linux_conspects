...
# ctrl + alt + F2  - выйти из консольного режима на экран логина, ctrl + alt + F1 на рабочий стол
...
# $ echo "folder done"  - оператор для вывода текстовых сообщений в терминал(можно без кавычек)
...
# 4. mv - позволяет переместить(вырезать и вставить) файл из одной папки в другую.
...
# 2. grep - команда для поиска информации внутри файлов или вывода в консоли, отображает строки, где были найдены совпадения. Искомый текст выведет выделенным красным, при поиске в нескольких фаилах выведет названия фаилов перед найденным в них текстом
