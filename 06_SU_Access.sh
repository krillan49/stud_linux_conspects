...
# w - команда которая показывает инфу обо всех пользователях, что подключены к серверу/нашему компу, время подключения итд
# $ w                   - показать инфу о пользователях

# Администратор в Linux называется супер пользователь(super user).

# Некоторые действия могут быть выполнены от имени администратора.

# sudo - (super user do) это ключевое слово нужно прописать перед кломандой для ее выполнения от имени администратора.
# При создании фаилов или папок от имени администратора(попросит пароль после нажатия Enter при первом обращении к sudo после первого обращения после открытия терминала), они будут иметь закрытые права доступа и пометку на иконке фаила. Это означает, что их редактирование и удаление может быть выполнено только супер пользователем.
...
# Можно перейти в режим администратора при помощи команды смены пользователя su, чтобы не писать перед каждой командой sudo

# Для некоторых дистрибутивов например UbuntuMATE возможно придется отдельно задать пароль скперпользователя, чтобы, описанный ниже функционал работал
...
# Где 1й символ(- фаил, d папка, l линк), далее права доступа для всех 3х типов(rw- r-- r--), далее хз что за цифра, дплее владелец(root) и группа(root), далее вес фаила в байтах, дата и его название
...
echo "                                   Смена прав и владельца фаила"


# 1. chown (change ownership) - команда для изменения владельца и группы фаила(владелец:группа). Если меняем фаил с владельцем root или на владельца root то нужно выполнять команду от имени суперпользователя (sudo). Чтобы сменить пользователя или группу папки нужно использовать допонительный флаг "-R"
...
# 2. chmod (change modification) - команда, чтобы менять права доступа к фаилу или папке, указывается в цифрах. Если права у фаила с владельцем root или на владельца root то нужно выполнять команду от имени суперпользователя (sudo). Чтобы сменить права доступа папки нужно использовать допонительный флаг "-R"
