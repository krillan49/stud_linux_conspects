...
# Терминал - это программа позволяющая запускать оболочку/shell например zsh или bash
...
# Вне зависимости от языка системы можно писать имена директорий по умолчанию на английском (например и "Загрузки" и "Downloads")
...
# $ whereis uptime        - выводит путь к местонахождению программы и местонахождению программы документации к ней
...
# ?? выведет какойто текст(с кавычками строка, без кавычек массив) ??
...
# Можно самостоятельно задать форматирование вывода. Форматирование задается после знака +. Первый символ после знака % интерпретируется как один из модификаторов. А остальные символы выводятся как обычный текст
# $ date +'%e.%m.%Y'                                                   - например тут день.месяц.год
# $ date +’%a %d.%m.%g %R’                                             - в кратком виде в 24-часовом формате
# $ date +'Какой сегодня день? Сегодня %e число %m месяца %Y года'
# Вывести дату и время в другом часовом поясе:
# $ TZ=America date
# $ TZ=America/New_York date                    - будет отличаться от того что выше хз почему
