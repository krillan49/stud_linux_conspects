...
# Пакеты - это приложения, они хранятся в репозитории в виде архивов и устанавливаются командами из терминала. Это более безопасный способ укачивания и установки приложений.
...
# 1. apt-get install - команда для скачивания и установки приложений, которые есть в стандартном репозитории, автоматически подтянет все зависимости устанавливаемого пакета
...
# 7. apt-get update  - команда для обновления информации о свежих версиях всего списка доступных пакетов и их версий из репозиториев. Система обращается к списку серверов указанных в фаиле "/etc/apt/sources.list" и других фаилах в каталоге "/etc/apt/sources.list.d/" и загружает с этих серверов информацию о доступных пакетах. Команда не устанавливает и не обновляет пакеты, она только обновляет локальный кэш информации о пакетах. После выполнения этой команды обычно используют "apt-get upgrade" для установки доступных обновлений, тк по сути для этого мы и обновили инфу
# $ sudo apt-get update
...
echo '                                          Aptitude (Debian)'

# Aptitude — оболочка для Advanced Packaging Tool (более крутой менеджер пакетов), части системы управления пакетами в операционной системе Debian и её производных. Имеет графический интерфейс (GTK), псевдографический и интерфейс командной строки.

# $ apt-get install aptitude                     - установить aptitude


# aptitude safe-upgrade - команда безопасного обновления всех установленных пакетов на свежие версии из репозитория. Обновляет все таким образом чтобы сохранить все зависимости и ни какие настройки не сломались, напрмер чтобы случайно не установился какой-то пакет у которого версия несовместима с предыдущней. ??Так же удаляет всякий не нужный хлам??
...
# Дополнительно curl может посылать HTTP запросы
