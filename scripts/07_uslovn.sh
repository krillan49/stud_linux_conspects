...
if [ "$num" -eq 5 ]; then # пробелы между скобками и условием обязятельны (можно больше 1 пробела для выравнивания)
  ...
  echo "jhgjhgh"
elif (( $1 % $2 -eq 0 && $1 % $3 -eq 0 )); then       # составное условие вариант 2 (какие-то ошибки с ||)
  printf 'true\n'
elif (( i%3==0 )) || (( i%5==0 )); then               # составное условие вариант 3 (мб корректнее чем выше)
  echo "jhgjh"
...
# Кавычки для переменных нужны, тк если переменная пуста, Bash обнаружит синтаксическую ошибку, тоесть будет так:
if [ = "valid" ]; then
fi
