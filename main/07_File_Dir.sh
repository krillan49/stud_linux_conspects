...
# 2. touch - команда позволяет создать один или несколько файлов(не может создавать папки). Если применить ее к существующему фаилу, то просто изменит время его создания(эта функция работает и с директориями).
...
# 3. mkdir - позволяет создать папки в определенной директории
...
# Для копирования директорий(со всем содержимым) нужно добавить флаг -R (либо -r)
# $ cp -R www new                         - копируется директория www под именем new в текущую директорию


# 5. mv - позволяет переместить(вырезать и вставить) файл из одной папки в другую, так же используется для переименования, что по сути одно и тоже, тк имя фаила или директории это конечная часть пути к этому фаилу или директории, тоесть изменение имени это и есть изменение пути
...
# type - команда выводит содержание скрипта ?? или это только для rvm ??
