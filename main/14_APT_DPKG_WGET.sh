...
# $ cat /etc/apt/sources.list       - посмотреть доки со ссылками на репозитории, которые использует apt-get

# APT(Advanced Package Tool) - система управления пакетами. Работает именно с программами из репозиториев
...
# 7. apt-get update  - команда для обновления списка доступных пакетов и их версий из репозиториев. При выполнении команды система обращается к серверам указанным в фаиле "/etc/apt/sources.list" и других фаилах в каталоге "/etc/apt/sources.list.d/" и загружает информацию о доступных пакетах. Команда не устанавливает и не обновляет пакеты, она только обновляет локальный кэш информации о пакетах. После выполнения этой команды обычно используют "apt-get upgrade" для установки доступных обновлений
# $ sudo apt-get update
