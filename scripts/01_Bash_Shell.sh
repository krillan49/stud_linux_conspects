...
# Bash (Bourne Again SHell) — это командный интерпретатор и язык сценариев, который используется в операционных системах на основе Unix (Linux, macOS итд). Bash позволяет выполнять команды в командной строке, а также писать автоматизированные скрипты - фаилы, содержащие команды и синтаксис языка. Язык сценариев Bash поддерживает переменные, условия, циклы, функции и другие конструкции
