...
# Debian дмтрибутивы(Ubuntu, Mint итд)                    - «apt-get».
# Red Hat Enterprise Linux дистрибутивы(CentOS итд)       - yum
...
echo "                                 DPKG - установщик программ от .deb(Debian)"
...
# На примере Гуглхром(но также можно скачать чтото через wget), скачиваем установочный фаил .deb с оф сайта гуглхрома(предложит вариант DEB - дебиан/убунту), жмем кнопку, выбиваем просто сохранить фаил, чтобы установить его вручную, а не при помощи графического интерфейса

# dpkg - команда установки из .deb при помощи DPKG
# -i   - (install) флаг значит что мы устанавливаем
# -r   - флаг значит что удаляем установленную программу
