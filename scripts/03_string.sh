...
# Чтобы рассплитить строку в массив не пол пробелу, а по другому символу, то спева нужно заменить его на пробел (?? правда хз тогда что делать с изначальными пробелами ??):
